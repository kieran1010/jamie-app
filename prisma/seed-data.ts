/**
 * Seed data and the two routines that write it.
 *
 * The split matters: reference data is what the app needs to function at all,
 * while demo data is a convenience for trying it out. A real deployment wants
 * the first on every build and the second never.
 */
import type { PrismaClient } from "@prisma/client";
import bcrypt from "bcryptjs";

/**
 * Password for every seeded demo account. Override it in the environment
 * before seeding anywhere the public can reach.
 */
...
/**
 * The subject and level lists the search filters are built from.
 *
 * This is reference data, not sample content: without it the filters render
 * empty and the app cannot be used. Idempotent — upserts by slug — so it is
...
/**
 * Demo tutors, a parent with two children, an adult learner and an admin.
 *
 * Keyed on email, so re-running refreshes rather than duplicates. The password
 * is rewritten on every run, not only on create: otherwise setting
 * DEMO_PASSWORD and re-seeding would leave existing accounts on the old
 * password while reporting the new one.
 *
 * Every account shares DEMO_PASSWORD, which is why this must not run unguarded
...
export async function seedDemo(prisma: PrismaClient): Promise<void> {
  ...
  for (const tutor of TUTORS) {
    const user = await prisma.user.upsert({
      ...
      update: { name: tutor.name, role: "TUTOR", passwordHash },
  ...
  const parent = await prisma.user.upsert({
    ...
    update: { name: "Sam Docherty", role: "SEEKER", passwordHash },
    create: {
  ...
  await prisma.user.upsert({
    ...
    update: { name: "Alex Reid", role: "SEEKER", passwordHash },
